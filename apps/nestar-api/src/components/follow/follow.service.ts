...
export class FollowService {
    ...
    public async subscribe(followerId: Types.ObjectId, followingId: Types.ObjectId): Promise<Follower> {
        if (followerId.toString() === followingId.toString()) {    // oziga ozi subscription bola olmaydi 
    ...
    private async registerSubscription(followerId: Types.ObjectId, followingId: Types.ObjectId): Promise<Follower> {
        try {
            ...
            console.log('Error, ServierModel', err.message);
            throw new BadRequestException(Message.CREATE_FAILED); // bu yerda error sodir bolishi mumkin shuning uchun biz ozimiz uchun CUSTOMIZED errorarni hosil qilib olyapmiz
        }
    }
            ...
            {$sort: { createdAt: Direction.DESC}},
            {
                $facet: {
                    list: [
                        ...
                        // "$followingId" = memberimiz following bolgan memberlarni following ID sini databasedan mana shu followingId orqali topadi
